...
(function( window, $, undefined ) {
  ...
  $.extend( MediaElementPlayer.prototype, {
    buildmeplike: function( player, controls, layers ) {
      ...
      $like.click(function() {
			var act  = 0;
			if($like.hasClass('is-like')) act = 1;
			else						  act = 0;
			setPlaylist(0, $like.attr( 'track-id'),act);
	...
	function setPlaylist(type, song_id, act){
		$.ajax({
		   type:'POST',
		   url:'/setplaylist',
	...
	//type: same db that is 0;playlist 1: favorite 2: downloaded
	//song_id
	// act : 0(unset)/1(set)
...
(function( window, $, undefined ) {
  ...
  $.extend( MediaElementPlayer.prototype, {
    ...
    createItem: function(item){
    	var tpl = '<li class="track"><div class="track-action"><a class="track-remove" data-id = "'+ item.id +'">&times;</a></div><div class="track-info"><span class="track-title">'+item.title+'</span><span class="track-author">'+item.meta.author+'</span></div></li>';
    	return tpl;
    },
